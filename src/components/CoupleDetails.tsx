...
export const CoupleDetails: React.FC = () => {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 relative">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-gradient-radial from-brand-lavender/15 to-transparent rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-8 relative z-10">
        ...
        >
          ...
          <div className="absolute -inset-2 bg-gradient-to-b from-brand-plum/30 to-transparent rounded-[12rem_12rem_1rem_1rem] blur-xl opacity-60 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />

          <div className="relative w-[300px] h-[400px] sm:w-[360px] sm:h-[500px] lg:w-[420px] lg:h-[580px] rounded-[12rem_12rem_16px_16px] overflow-hidden border-[6px] border-white/90 shadow-[0_20px_50px_rgba(176,137,104,0.2)] bg-brand-rose mx-auto z-10 transition-transform duration-700 group-hover:-translate-y-2">
            <img
              src="/1.jpeg"
              loading="eager"
              decoding="async"
              alt="Shashika and Malsha"
              className="w-full h-full object-cover transition-transform duration-[1.5s] ease-out group-hover:scale-105"
            />
            {/* Elegant inner shadow & overlay */}
            <div className="absolute inset-0 shadow-[inset_0_0_40px_rgba(0,0,0,0.1)] pointer-events-none" />
            <div className="absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-brand-plum/30 to-transparent pointer-events-none mix-blend-overlay" />
          </div>

          {/* Floating Sparkles decoration */}
          <Sparkles className="absolute top-8 left-0 w-8 h-8 text-brand-plum animate-pulse drop-shadow-sm z-20" />
          <Sparkles className="absolute bottom-10 -right-4 w-6 h-6 text-brand-lavender animate-pulse delay-300 drop-shadow-sm z-20" />
        </motion.div>

        {/* Mobile divider between image and groom */}
        <div className="lg:hidden order-3 w-full flex justify-center py-4 opacity-50">
          <div className="w-1/2 h-px bg-gradient-to-r from-transparent via-brand-plum/40 to-transparent" />
        </div>
